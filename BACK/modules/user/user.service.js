...
async function getLeaderboard(userId, gameId, sortBy = 'score') {
    const user = await AppUser.findByPk(userId, {
        include: { model: UserPlan, attributes: ['planType'] }
    });
    ...
    if (planType === 'BASIC') {
        ...
        return {
            type: 'BASIC',
            data: ownScore
                ? [{ nickname: person.nickname, score: ownScore.totalPoints, date: ownScore.lastUpdate }]
                : []
        };
    }
    ...
    const ranking = await Leaderboard.findAll({
        ...
        include: {
            model: AppUser,
            include: { model: Person, attributes: ['nickname'] }
        }
    });
...
async function updateLeaderboard(userId, gameId, score) {
    ...
    await createEvent({
        ...
        category: 'GAME_MANAGEMENT',
        description: `Leaderboard actualizado en juego ${gameId}`
    });
...
async function createMatch(userId, gameId) {
    ...
    try {
        ...
        if (!userGame.active) {
            throw new Error('You have already played this game. Try again tomorrow.');
        }
        ...
        await createEvent({
            ...
            category: 'GAMEPLAY',
            description: `Partida iniciada en juego ${gameId}`
        });

        await transaction.commit();

        return { match: todayMatch, streak };
...
async function finishMatch(matchId, score, extraData) {
    ...
    try {
        ...
        const matchesToday = await GameMatch.findAll({
            where: {
                userId: match.userId,
                date: { [Op.gte]: getToday(), [Op.lt]: getTomorrow() }
            },
            transaction
        });
        ...
        const matchesYesterday = await GameMatch.findAll({
            where: {
                userId: match.userId,
                date: { [Op.gte]: getYesterday(), [Op.lt]: getToday() }
            },
            transaction
        });
        ...
        if (
            uniqueGamesToday.length === 4 &&
            (uniqueGamesYesterday.length === 4 || isFirstDay || isLastDayOfMonth)
        ) {
            const [reward, created] = await DailyGameReward.findOrCreate({
                where: { userId: match.userId, rewardDate: getToday() },
                defaults: {
                    totalScore: matchesToday.reduce((sum, m) => sum + m.score, 0)
                },
                transaction
            });
...
async function getHangmanWords() {
    return await GameWord.findAll({
        where: { gameId: 1, active: true },
        attributes: ['id', 'word', 'language'],
        order: [['id', 'ASC']]
    });
}
